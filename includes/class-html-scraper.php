...
class Html_Scraper {
	...
	public function scrape_url( $url, $target_class = '' ) {
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Request failed: %s', 'better-media-manager' ),
		...
		if ( 200 !== $status_code ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'HTTP error %d: Could not fetch the page.', 'better-media-manager' ),
					$status_code
				)
			);
		}
		...
		if ( empty( $html ) ) {
			return new \WP_Error( 'empty_response', __( 'Received empty response from server.', 'better-media-manager' ) );
		}
