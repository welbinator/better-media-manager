...
class Media_Importer {
		...
		foreach ( $images as $index => $image ) {
			...
			if ( ! is_wp_error( $result ) ) {
				...
				$this->errors[] = sprintf(
					...
					__( '%1$s: %2$s', 'better-media-manager' ),
					isset( $image['filename'] ) ? $image['filename'] : __( 'Unknown file', 'better-media-manager' ),
	...
	private function import_single_image( $image ) {
		if ( empty( $image['url'] ) ) {
			return new \WP_Error( 'no_url', __( 'No image URL provided.', 'better-media-manager' ) );
		}
		...
		if ( is_wp_error( $temp_file ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Could not download %1$s - %2$s', 'better-media-manager' ),
					esc_url( $image['url'] ),
		...
		if ( is_wp_error( $editor ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Could not load image: %s', 'better-media-manager' ),
		...
		if ( $current_size > $max_size_bytes ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Could not compress to %2$d KB (best achieved: %1$d KB).', 'better-media-manager' ),
	...
	private function download_with_retry( $url, $max_retries = 3 ) {
		...
		return new \WP_Error(
			...
			sprintf(
				...
				__( 'Failed after %1$d attempts: %2$s', 'better-media-manager' ),
				$max_retries,
