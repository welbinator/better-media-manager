...
class Firecrawl_Api {
	...
	public function validate_api_key() {
		if ( empty( $this->api_key ) ) {
			return new \WP_Error( 'no_api_key', __( 'API key is empty.', 'better-media-manager' ) );
		}

		// Simple validation - check if it's not empty and has reasonable length.
		if ( strlen( $this->api_key ) < 10 ) {
			return new \WP_Error( 'invalid_api_key', __( 'API key appears to be invalid (too short).', 'better-media-manager' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Connection failed: %s', 'better-media-manager' ),
	...
	public function scrape_url( $url, $target_class = '' ) {
		...
		if ( empty( $this->api_key ) || strlen( $this->api_key ) < 10 ) {
			return new \WP_Error( 'no_api_key', __( 'Firecrawl API key is not configured.', 'better-media-manager' ) );
		}

		if ( empty( $url ) || ! filter_var( $url, FILTER_VALIDATE_URL ) ) {
			return new \WP_Error( 'invalid_url', __( 'Invalid URL provided.', 'better-media-manager' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			return new \WP_Error(
				...
				sprintf(
					/* translators: %s: error message */
					__( 'API request failed: %s', 'better-media-manager' ),
					$response->get_error_message()
				)
			);
		}
		...
		if ( ! isset( $data['data'] ) || ! isset( $data['data']['html'] ) ) {
			return new \WP_Error( 'invalid_response', __( 'Invalid API response format.', 'better-media-manager' ) );
		}
