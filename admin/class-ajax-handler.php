...
class Ajax_Handler {
	...
	public function handle_scrape() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error(
				array( 'message' => __( 'You do not have permission to perform this action.', 'better-media-manager' ) )
			);
		}
		...
		if ( empty( $target_url ) || ! filter_var( $target_url, FILTER_VALIDATE_URL ) ) {
			wp_send_json_error(
				array( 'message' => __( 'Please provide a valid URL.', 'better-media-manager' ) )
			);
		}
		...
		if ( 'firecrawl' === $method ) {
			...
			$scraper = new \Better_Media_Manager\Html_Scraper();
		}
	...
	public function handle_validate_api() {
		...
		// Initialize Firecrawl API.
		$api = new \Better_Media_Manager\Firecrawl_Api();
		...
		// Return success.
		wp_send_json_success(
			array( 'message' => __( 'API key is valid and working!', 'better-media-manager' ) )
		);
	}
	...
	public function handle_import() {
		...
		if ( empty( $images ) || ! is_array( $images ) ) {
			wp_send_json_error(
				array( 'message' => __( 'No images provided for import.', 'better-media-manager' ) )
			);
		}
		...
		$importer = new \Better_Media_Manager\Media_Importer( $global_options );
