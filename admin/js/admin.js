...
(function( $ ) {
	...
	$(document).ready(function() {
		...
		$('#test-api-key').on('click', function() {
			...
			// Make AJAX request.
			$.ajax({
				url: betterMediaManagerAdmin.ajax_url,
				type: 'POST',
				data: {
					action: 'better_media_manager_validate_api',
					nonce: betterMediaManagerAdmin.nonce
				},
	 ...
	 */
	$('#better-media-manager-form').on('submit', function(e) {
		e.preventDefault();
		...
		var targetClass = $('#target_class_toggle').is(':checked') ? $('#target_class').val() : '';			// Validate URL.
			if (!targetUrl) {
			...
			// Prepare data.
			var data = {
				action: 'better_media_manager_scrape',
				nonce: betterMediaManagerAdmin.nonce,
		...
		$('#import-options-form').on('submit', function(e) {
			...
			var data = {
				action: 'better_media_manager_import',
				nonce: betterMediaManagerAdmin.nonce,
		...
		$('#start-over').on('click', function() {
			...
			$('#better-media-manager-form')[0].reset();
			...
			// Scroll to top.
			$('html, body').animate({
				scrollTop: $('.better-media-manager-form-wrapper').offset().top - 50
			}, 500);
		});
